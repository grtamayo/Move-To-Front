/*
Filename:     gtmtf4.c, move-to-front
...
void usage( void )
{
	fprintf(stderr, "\n Usage: gtmtf4 c|d infile outfile\n"
		"\n Commands:\n  c = Encoding.\n  d = Decoding.\n"
	);
	copyright();
	exit(0);
...
int main( int argc, char *argv[] )
{
	...
	alloc_mtf(MTF_SIZE);
	block = (unsigned char *) malloc( sizeof(unsigned char) * BLOCK_SIZE );
	if ( !block ) {
...
void compress( unsigned char * block )
{
	...
	while ( (nread=fread(block, 1, BLOCK_SIZE, gIN)) ) {
		n = 0;
		while ( n < nread ){
			c = *(block+n);
			pfputc( mtf(c) );
